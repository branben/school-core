...
| **F** | `executor.py` COMBO_MAP (`auto/best-free`) orphaned | Duplicates OmniRoute routing that the live path ignores. | ✅ **Wired in via ACRouter (P2.1):** `executor.call_model` now routes through `router_experience.RouterExperience` — an epsilon-greedy bandit that selects the combo per role and learns from recorded success/quality outcomes (the two-judge review verdict). The static `COMBO_MAP` is the cold-start default until experience accumulates. Outcome feedback recorded in `director.run_task`. See `docs/adr/acrouter-combo-routing.md`. |
