...
| **C** | `conductor.py --serve` → `run_principal_loop.py` (`while True` pane) | Manual loop in a terminal — the #1 "won't stand up in Orca" failure mode. | Replace with `orca automations create --provider hermes` (Orca owns schedule). |
| **D** | `_boot_teachers` → `terminal send "python3 run_teacher_loop.py"` | Fragile launcher; empty-terminal trap. | `orca worktree create --name teacher-cto --agent hermes --prompt "<review loop>"` (one native call). |
